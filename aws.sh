...
echo "installing aws s3 example"
wget --quiet http://s3.amazonaws.com/doc/s3-example-code/s3-curl.zip
unzip -qq s3-curl.zip
sudo mkdir -p /usr/local/aws/s3/bin/
sudo rsync -a --no-o --no-g s3-curl/ /usr/local/aws/s3/bin/
...
nano aws-credential-file.txt
echo "AWSAccessKeyId=<insert your AWS access id here>" >> aws-credential-file.txt 
echo  "AWSSecretKey=<insert your AWS secret access key here>" ~>> aws-credential-file.txt
...
for i in $EC2_HOME $AWS_IAM_HOME $AWS_RDS_HOME $AWS_ELB_HOME  $AWS_CLOUDFORMATION_HOME $AWS_AUTO_SCALING_HOME $CS_HOME   $AWS_CLOUDWATCH_HOME $AWS_ELASTICACHE_HOME $AWS_SNS_HOME  $AWS_ROUTE53_HOME $AWS_CLOUDFRONT_HOME 
/usr/local/aws/s3
do
  PATH=$i/bin:$PATH
done
